(function() {"use strict";var __module = CC_EDITOR ? module : {exports:{}};var __filename = 'preview-scripts/assets/scripts/app/game/GameMgr.js';var __require = CC_EDITOR ? function (request) {return cc.require(request, require);} : function (request) {return cc.require(request, __filename);};function __define (exports, require, module) {"use strict";
cc._RF.push(module, 'c4208KSHvVLipykrEwvZ/oT', 'GameMgr', __filename);
// scripts/app/game/GameMgr.js
...
cc.Class({
  ...
  onGameStateChanged: function onGameStateChanged(ret) {
  ...
  //开始挤牌
  on_room_game_squeeze_cards_square: function on_room_game_squeeze_cards_square(ret) {
  ...
  //挤牌位置同步
  on_room_game_squeeze_cards_pos: function on_room_game_squeeze_cards_pos(ret) {
  ...
  //挤牌时下发
  on_room_game_squeeze_cards: function on_room_game_squeeze_cards(ret) {
    //翻开所有牌
    if (null == ret.index) {
      for (var i = 1; i <= 2; i++) {
  ...
  checkHideFlipBtn: function checkHideFlipBtn() {
  ...
  //比牌
  on_room_game_compare_cards: function on_room_game_compare_cards(ret) {
    this.view.compareCards(ret);
  },

  on_room_game_send_cards: function on_room_game_send_cards(ret) {
    if (0 == this.curJushu) {
  ...
  on_room_game_mend_cards: function on_room_game_mend_cards(ret) {
    this.view.buCards(ret);
  },

  //下注
  on_room_game_betting: function on_room_game_betting(ret) {
  ...
  settle: function settle(ret, isMiddle) {
    for (var i = 1; i <= 9; i++) {
  ...
  //单局游戏结算
  on_settle_accounts_inning: function on_settle_accounts_inning(ret) {
    this.settle(ret);
  ...
  //中途结算
  on_settle_accounts_midway: function on_settle_accounts_midway(ret) {
    this.settle(ret, true);
  },

  //游戏总结算
  on_settle_accounts_finish: function on_settle_accounts_finish(ret) {
    var view = this.view.openView("prefabs/totalResult/TotalResultView");
    view.setData(clone(ret));
  },

  onRoomLeave: function onRoomLeave(ret) {
  ...
  onRoomJoin: function onRoomJoin(ret) {
  ...
  on_room_game_cut_cards: function on_room_game_cut_cards(ret) {
  ...
  getSeatDataByUID: function getSeatDataByUID(uid) {
  ...
  setRoomOwner: function setRoomOwner(ownerSeatData) {
  ...
  isRoomOwner: function isRoomOwner() {
    return this._isRoomOwner;
  },

  //计算UI上的座位ID
  calSeatId: function calSeatId(myId, otherId) {
    var ownerOffset = 0;
    if (!this.isRoomOwner()) {
      ownerOffset = 1;
    }
  ...
  //设置玩家数据
  setPlayerDatas: function setPlayerDatas(playerDatas) {
  ...
  reqRoomLeave: function reqRoomLeave() {
  ...
  reqRoomClose: function reqRoomClose() {
    cc.netMgr.request("room_close_launch", {}, function (ret) {
      cc.netMgr.exec(ret, function () {});
    });
  },

  reqStartGame: function reqStartGame() {
  ...
  //切牌
  reqCutCards: function reqCutCards(data) {
  ...
  //清除下注
  reqClearBet: function reqClearBet() {
  ...
  //停止下注
  reqStopBet: function reqStopBet() {
  ...
  handleRoomInfo: function handleRoomInfo() {
    //设置时间差
    var curTime = Date.parse(new Date()) / 1000;
  ...
  //转移挤牌权
  reqTransCardPermisssion: function reqTransCardPermisssion(data) {
  ...
  on_room_game_trans_permi: function on_room_game_trans_permi(ret) {
  ...
  showPowerObjTimer: function showPowerObjTimer() {
    //有挤牌权的人显示倒计时
    for (var i = 1; i <= 9; i++) {
      var player = this.view.find("player" + i);
      if (player.data) {
  ...
  on_room_chat: function on_room_chat(ret) {
    var player = this.view.getPlayerByUid(ret.uid);
    player.showBubble({ type: ret.type, words: ret.words });
  },

  setJushu: function setJushu(num) {
    this.curJushu = num;
    var totalJushu = 0;
  ...
  on_room_game_clear_bet: function on_room_game_clear_bet(ret) {
    var player = this.view.getPlayerByUid(ret.uid);
  ...
  //获取挤牌类型
  getSqueezeType: function getSqueezeType() {
    //浏览器暂不支持3D挤牌
    if (cc.sys.isBrowser) {
  ...
  //切牌杆位置同步
  on_room_game_cut_cards_pos: function on_room_game_cut_cards_pos(ret) {
  ...
  //检查是否有延时挤牌要处理
  checkDelaySqueeze: function checkDelaySqueeze() {
